...
def pluralize(sg):
    ...
    if sg in ['auto', 'kangaroo', 'kilo', 'memo', 'photo', 'piano', 'pimento',
    ...
    elif sg in ['fish', 'sheep', 'barracks', 'deer']:
        plurals.append(sg)
    ...
    elif sg in ['focus', 'nucleus', 'radius', 'stimulus', 'terminus',
        'alumnus']:
        ...
        plurals.append(sg[:-2] + 'ora')
    elif sg == 'genus':
        plurals.append(sg[:-2] + 'era')
    elif sg == 'bacillus':
        plurals.append(sg[:-2] + 'i')
    elif sg in ['cactus', 'fungus', 'octopus', 'syllabus']:
        plurals.append(sg[:-2] + 'i')
        plurals.append(sg + 'es')
    elif sg in ['addendum', 'bacterium', 'datum', 'erratum', 'medium', 'ovum',
        'stratum']:
        plurals.append(sg[:-2] + 'a')
    elif sg in ['memorandum', 'symposium', 'curriculum']:
        plurals.append(sg[:-2] + 'a')
        plurals.append(sg + 's')
        ...
        plurals.append('')
    elif sg in ['apex', 'appendix', 'cervix', 'index', 'matrix']:
        plurals.append(sg + 'es')
        plurals.append(sg[:-2] + 'ices')
    elif sg[-2:] == 'is':
